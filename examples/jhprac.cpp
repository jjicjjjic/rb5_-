...
Vector3d g_current_pos = Vector3d::Zero();
Vector3d g_current_rpy = Vector3d::Zero();
...
void sendCurrentPoseToGUI() {
    if (client_fd < 0) {
        std::cout << "[TCP-feedback] client_fd < 0, 전송 안 함\n";
        return;
    }
    ...
    json j;
    {
        ...
        j["y"]     = g_current_pos[1];
        j["z"]     = g_current_pos[2];
        j["roll"]  = rad2deg(g_current_rpy[0]);
        j["pitch"] = rad2deg(g_current_rpy[1]);
        j["yaw"]   = rad2deg(g_current_rpy[2]);
    }
...
void tcpReceiveThread() {
    ...
    while (running.load()) {
        ...
        try {
            ...
            std::cout << "[DEBUG] g_current_pos: " << g_current_pos.transpose() << std::endl;
            std::cout << "[DEBUG] g_current_rpy: " << g_current_rpy.transpose() << std::endl;
...
int main() {
    try {
        ...
        while (running.load()) {
            ...
            {
                lock_guard<mutex> lock(data_mutex);
                g_current_pos = T.block<3,1>(0,3);
                g_current_rpy = rm.matrixToRPY(T);
            }
            sendCurrentPoseToGUI();
