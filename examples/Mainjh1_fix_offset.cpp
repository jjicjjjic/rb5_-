...
void executeStaticMove(
    ...
    double ori_diff = start_q.angularDistance(target_q);  // Quaternion 간 회전 거리

    if (total_len < 1e-3 && ori_diff < 1e-3) return;  // 둘 다 변화 없으면 skip
    if (total_len < 1e-3) total_len = 100.0 * ori_diff;  // 위치 변화는 없지만 방향은 바뀐 경우
...
int main() {
    try {
        ...
        Vector3d Base_Pose(413, 110, 378);
        Vector3d Base_RPY(deg2rad(90), deg2rad(0), deg2rad(65));
