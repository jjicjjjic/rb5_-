...
void executeStaticMove(
    ...
    for (int i = 0; i < N; ++i) {
        ...
        // 5.3) Alpha 선형 보간
        constexpr double αmin = 0.05, αmax = 0.8;
        double alpha = αmin + (αmax - αmin) * (prog * prog * prog);
...
int main() {
    try {
        ...
        Vector3d prev_pos = Vector3d::Zero();
        Quaterniond prev_q = Quaterniond::Identity();
        while (running.load()) {
            ...
            // 이전 명령과 동일하면 또 움직이지 않음
            if ((pos - prev_pos).norm() < 1.0 && target_q.angularDistance(prev_q) < 0.05) {
