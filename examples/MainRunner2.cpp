...
int main() {
    ...
    try {
        ...
        // std::string line;
        // while (true) {
        //     std::cout << "\nEnter target x y z [mm], roll pitch yaw [deg] (or 'q' to quit): ";
        //     std::getline(std::cin, line);
        //     if (line == "q" || line.empty()) break;

        //     std::istringstream iss(line);
        //     double x, y, z, rolld, pitchd, yawd;
        //     if (!(iss >> x >> y >> z >> rolld >> pitchd >> yawd)) {
        //         std::cout << "  입력 형식 오류 — 다시 시도하세요.\n";
        //         continue;
        //     }

        //     // 목표 위치·자세 구성
        //     Vector3d target_pos(x, y, z);
        //     // Z→Y→X 순서로 쿼터니언 생성
        //     Quaterniond qx(AngleAxisd(rolld * DEG2RAD, Vector3d::UnitX()));
        //     Quaterniond qy(AngleAxisd(pitchd * DEG2RAD, Vector3d::UnitY()));
        //     Quaterniond qz(AngleAxisd(yawd * DEG2RAD, Vector3d::UnitZ()));
        ...
        std::vector<Vector3d> orientations_deg = {
            {90.0, 0.0, 0.0},
            {30.0, 0.0, 0.0},
            {30.0, 0.0, 0.0},
            {60.0, 0.0, 0.0},
        ...
        for (size_t i = 0; i < positions.size(); ++i) {
            Vector3d target_pos = positions[i];
            Vector3d rpy_deg = orientations_deg[i];

            // Z→Y→X 순서로 쿼터니언 생성
            Quaterniond qx(AngleAxisd(rpy_deg[0] * DEG2RAD, Vector3d::UnitX()));
            Quaterniond qy(AngleAxisd(rpy_deg[1] * DEG2RAD, Vector3d::UnitY()));
            Quaterniond qz(AngleAxisd(rpy_deg[2] * DEG2RAD, Vector3d::UnitZ()));
            Quaterniond target_q = qz * qy * qx;

            std::cout << "→ [" << i + 1 << "]번째 목표점으로 이동 중...\n";

            // 정적 경로로 이동
            executeStaticMove(rm, robot, rc, q_current, target_pos, target_q);
            std::cout << "→ 목표점에 도달했습니다. 다음 지점을 입력하세요.\n";
