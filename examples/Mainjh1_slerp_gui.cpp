...
Vector3d g_target_rpy = Vector3d::Zero();
...
void sendCurrentPoseToGUI() {
    ...
    // 1. Raw pose → Homogeneous transform
    Matrix4d T_base2tcp = Matrix4d::Identity();
    Quaterniond qx(AngleAxisd(rpy_raw[0], Vector3d::UnitX()));
    Quaterniond qy(AngleAxisd(rpy_raw[1], Vector3d::UnitY()));
    Quaterniond qz(AngleAxisd(rpy_raw[2], Vector3d::UnitZ()));
    Matrix3d R = (qz * qy * qx).toRotationMatrix();
    T_base2tcp.block<3,3>(0,0) = R;
    T_base2tcp.block<3,1>(0,3) = pos_raw;
    ...
    // 4. 변환행렬 → 보정된 위치 + RPY
    Vector3d pos_corr = T_tool.block<3,1>(0,3);

    // ZYX 기준 RPY 추출
    Vector3d rpy_corr;
    Matrix3d R_corr = T_tool.block<3,3>(0,0);
    rpy_corr[1] = -asin(R_corr(2,0));
    double c_pitch = cos(rpy_corr[1]);
    if (fabs(c_pitch) > 1e-6) {
        rpy_corr[0] = atan2(R_corr(2,1), R_corr(2,2));  // roll
        rpy_corr[2] = atan2(R_corr(1,0), R_corr(0,0));  // yaw
    } else {
        rpy_corr[0] = 0.0;
        rpy_corr[2] = atan2(-R_corr(0,1), R_corr(1,1));
    }
    ...
    json j;
    {
        ...
        j["y"]     = pos_corr[1];
        j["z"]     = pos_corr[2];
        j["roll"]  = rad2deg(rpy_corr[0]);
        j["pitch"] = rad2deg(rpy_corr[1]);
        j["yaw"]   = rad2deg(rpy_corr[2]);
    }
...
void tcpReceiveThread() {
    ...
    // 2) 서버 주소 설정 (Python server: localhost:12345)
    struct sockaddr_in serv_addr;
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port   = htons(PORT);
    if (inet_pton(AF_INET, "127.0.0.1", &serv_addr.sin_addr) <= 0) {
        std::cerr << "[TCP] Invalid address/ Address not supported\n";
        close(client_fd);
        return;
    }

    // 3) 서버에 연결 시도
    std::cout << "[TCP] Connecting to Python server at 127.0.0.1:" << PORT << " ...\n";

    
    while (connect(client_fd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) {
        // 연결에 실패하면 잠시 대기 후 재시도
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    std::cout << "[TCP] Connected to Python server.\n";

    std::cout << "[TCP] connect 성공, 이제 feedbackThread 띄웁니다\n";

     std::thread feedbackThread([&]() {
        // 매 100ms마다 현재 로봇 위치/자세를 JSON 으로 보내라
        std::cout << "[TCP-feedback] feedbackThread 시작\n";
        while (running.load()) {
            sendCurrentPoseToGUI();
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }
        std::cout << "[TCP-feedback] feedbackThread 종료\n";
    });
    feedbackThread.detach();

    std::cout << "[TCP] recv-loop 진입 전\n";
    // 4) 연결이 수립되었으면, 이후로는 recv() 루프만 실행
    while (running.load()) {
        ...
        std::cout << "[TCP DEBUG] Raw received: " << recv_buffer << "\n";

        // 만약 Stop 신호('s')인 경우
        if (recv_buffer == "s") {
            // 로봇 모션 종료 코드
            // 🔽 로봇 모션 종료 코드 추가
            rb::podo::ResponseCollector response_collector;
            rb::podo::ReturnType result = g_robot_ptr->task_stop(response_collector, 2.0);

            if (result.is_success()) {
                std::cout << "[TCP] ✅ Task stopped successfully." << std::endl;
            } 
            break;
        }

        try {
            // JSON 전체가 들어왔다고 가정하고 파싱
            json j = json::parse(recv_buffer);
            recv_buffer.clear();

            Vector3d pos(j["x"], j["y"], j["z"]);
            Vector3d rpy(deg2rad(j["roll"]), deg2rad(j["pitch"]), deg2rad(j["yaw"]));
            ...
            std::cerr << "[TCP] Parse error: " << e.what() << "\n";
        }
    }
...
public:
    ...
    Eigen::Matrix4d forwardKinematics(const vector<double>& jointVals) {
        ...
        T_offset(0, 3) = 0;  // y異뺤쑝濡� -110mm �대룞
        T_offset(1, 3) = -134.5;  // y異뺤쑝濡� -110mm �대룞
        T_offset(2, 3) = -134.5;  // y異뺤쑝濡� -110mm �대룞
        T = T * T_offset;
    ...
    Matrix<double, 6, 6> computeJacobian6x6(const vector<double>& jointVals) {
        ...
        Matrix4d T_end = T_list[9]; 
        Eigen::Matrix4d T_offset = Matrix4d::Identity();
        T_offset(0, 3) = 0;  // y異뺤쑝濡� -110mm �대룞
        T_offset(1, 3) = -134.5;  // y異뺤쑝濡� -110mm �대룞
        T_offset(2, 3) = -134.5;  // y異뺤쑝濡� -110mm �대룞
        T_end = T_end * T_offset; // 기존 TCP 위치
...
void executeStaticMove(
    ...
    double ori_diff = start_q.angularDistance(target_q);

    // 3) 보간 조건 판단
    bool only_rpy = (total_len < 10.0 && ori_diff > deg2rad(10.0));

    int N;
    if (only_rpy) {
        N = int(2.0 / LOOP_DT);  // RPY만 바뀐 경우 2초간 보간
        std::cout << "[DEBUG] RPY only. Using fixed 2s SLERP.\n";
    }
    else {
              N = std::max(1, int(total_len));  // 위치 기반
    };
...
int main() {
    try {
        std::thread tcpThread(tcpReceiveThread);
        Robot9Link rm;
        podo::Cobot robot("10.0.2.7");
        podo::ResponseCollector rc;
        std::array<double, 6> pnt{};

        // Vector3d last_target_pos = Vector3d::Zero();
        ...
        while (running.load()) {
            ...
            // double pos_diff = (pos - last_target_pos).norm();
            // double ori_diff = (rpy - last_target_rpy).norm();
            // if (pos_diff < 1.0 && ori_diff < deg2rad(2.0)) {
            //     std::cout << "[INFO] Target nearly identical. Skipping.\n";
            ...
            std::cout << "[SYNC] Re-reading current joint angles before move." << std::endl;
            for (int i = 0; i < 6; ++i) {
                robot.get_system_variable(rc,
                    static_cast<podo::SystemVariable>(static_cast<int>(podo::SystemVariable::SD_J0_REF) + i), out);
                q_current[i] = out * DEG2RAD;
