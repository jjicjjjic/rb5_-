...
void tcpCommunicationThread() {
    ...
    std::cout << "TCP server is waiting on Port " << CONTROL_PORT << "." << std::endl;
...
public:
        ...
        double eps = 1e-3)
    {
        ...
        for (int iter = 0; iter < maxIter; iter++) {
            ...
            if(wrapAngleDeg(q[0]*RAD2DEG)>=-180 && wrapAngleDeg(q[0]*RAD2DEG)<=0){err1 = 0;}else{err1 = 100;}
            if(wrapAngleDeg(q[1]*RAD2DEG)>=-45 && wrapAngleDeg(q[1]*RAD2DEG)<=45){err2 = 0;}else{err2 = 100;}
            if(wrapAngleDeg(q[2]*RAD2DEG)>=-150 && wrapAngleDeg(q[2]*RAD2DEG)<=0){err3 = 0;}else{err3 = 100;}
            // if(wrapAngleDeg(q[3]*RAD2DEG)>=90 && wrapAngleDeg(q[3]*RAD2DEG)<=180){err4 = 0;}else{err4 = 1;}
            // if(wrapAngleDeg(q[4]*RAD2DEG)>=90 && wrapAngleDeg(q[4]*RAD2DEG)<=180){err5 = 0;}else{err5 = 1;}
            // if(wrapAngleDeg(q[5]*RAD2DEG)>=90 && wrapAngleDeg(q[5]*RAD2DEG)<=180){err6 = 0;}else{err6 = 1;}
...
int main() {
        ...
        for(int i=0;i<6;i++){
            BA[i] = wrapAngleDeg(BA[i]*RAD2DEG);
        }
        ...
        constexpr double MIN_DISTANCE = 10.0; // mm
        // 오차 누적: pos_x, pos_y, pos_z, roll, pitch, yaw
        double sum_err_pos[3] = { 0,0,0 };
        double sum_err_ori[3] = { 0,0,0 };

        std::mt19937_64 rng(std::random_device{}());
        std::uniform_real_distribution<double> dist_x(-100.0, 100.0);
        std::uniform_real_distribution<double> dist_yz(300.0, 500.0);
        std::uniform_real_distribution<double> dist_r(0, 90.0);
        std::uniform_real_distribution<double> dist_rpy(-20.0, 20.0);
        
        // (2) Planning
        std::vector<double> q_current(6);
        // 현재 관절값 읽어오기 (초기 q_current 세팅)
        for (int i = 0; i < 6; ++i) {
            double deg;
            robot.get_system_variable(rc,
                static_cast<podo::SystemVariable>(
                    static_cast<int>(podo::SystemVariable::SD_J0_REF) + i
                    ), deg);
        ...
        for (int t = 0; t < TRIALS; ++t) {
            ...
            // (1) 랜덤 목표 생성
            Vector3d target_pos{ dist_x(rng), dist_yz(rng), dist_yz(rng) };
            double roll_d = dist_r(rng),
                pitch_d = dist_rpy(rng),
                yaw_d = dist_rpy(rng);
            Quaterniond target_q =
                AngleAxisd(yaw_d * DEG2RAD, Vector3d::UnitZ()) *
                AngleAxisd(pitch_d * DEG2RAD, Vector3d::UnitY()) *
                AngleAxisd(roll_d * DEG2RAD, Vector3d::UnitX());

            // (2) 현재 TCP 위치 읽기
            std::array<double, 6> curinfo;
            robot.get_tcp_info(rc, curinfo);
            Vector3d cur_pos{ curinfo[0], curinfo[1], curinfo[2] };

            // (3) 너무 가까우면 재시도
            if ((target_pos - cur_pos).norm() < MIN_DISTANCE) {
                --t;
                continue;
            }
            // b) 실행
            executeStaticMove(rm, robot, rc, q_current, target_pos, target_q);

            // c) 실제 멈춘 위치·자세 읽기
            std::array<double, 6> tcpinfo;
            robot.get_tcp_info(rc, tcpinfo);
            Vector3d actual_pos{ tcpinfo[0], tcpinfo[1], tcpinfo[2] };
            Vector3d actual_rpy = quaternionToRPY(
                AngleAxisd(tcpinfo[3] * DEG2RAD, Vector3d::UnitX()) *
                AngleAxisd(tcpinfo[4] * DEG2RAD, Vector3d::UnitY()) *
                AngleAxisd(tcpinfo[5] * DEG2RAD, Vector3d::UnitZ())
            );

            // d) 오차 계산 (절댓값)
            Vector3d err_pos = (actual_pos - target_pos).cwiseAbs();
            Vector3d err_ori = (actual_rpy - Vector3d(roll_d, pitch_d, yaw_d) * DEG2RAD).cwiseAbs();

            for (int i = 0; i < 3; ++i) {
                sum_err_pos[i] += err_pos[i];
                sum_err_ori[i] += err_ori[i];
            }

            std::cout << "[Trial " << t + 1 << "] "
                << "pos_err(mm)=[" << err_pos.transpose() << "] "
                << "ori_err(deg)=["
                << rad2deg(err_ori[0]) << ","
                << rad2deg(err_ori[1]) << ","
                << rad2deg(err_ori[2]) << "]\n";
        ...
        // 3) 평균 오차 출력
        std::cout << "\n=== 평균 오차 over " << TRIALS << " trials ===\n";
        std::cout << "  pos_err(mm) avg = ["
            << (sum_err_pos[0] / TRIALS) << ", "
            << (sum_err_pos[1] / TRIALS) << ", "
            << (sum_err_pos[2] / TRIALS) << "]\n";
        std::cout << "  ori_err(deg) avg = ["
            << rad2deg(sum_err_ori[0] / TRIALS) << ", "
            << rad2deg(sum_err_ori[1] / TRIALS) << ", "
            << rad2deg(sum_err_ori[2] / TRIALS) << "]\n";
